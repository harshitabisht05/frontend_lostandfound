...
import { MenuIcon, CompassIcon } from "./Icons";

export default function Navbar() {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [isLoggedIn, setIsLoggedIn] = useState(false);

  // Check login state on page load (from localStorage, cookie, etc.)
  useEffect(() => {
    const token = localStorage.getItem("authToken"); // or use your own key
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-opacity-30 backdrop-blur-md border-b border-slate-700/50">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <a href="/" className="flex items-center space-x-2 text-2xl font-bold text-white">
          <CompassIcon className="w-7 h-7 gradient-text" />
        ...
        <div className="hidden md:flex items-center space-x-4">
          {!isLoggedIn ? (
            ...
            <button
              ...
              className="bg-slate-700 text-white font-semibold px-4 py-2 rounded-lg hover:bg-slate-600 transition-colors duration-300"
            >
              Logout
            </button>
          )}
      ...
      <div className={`${isMenuOpen ? "block" : "hidden"} md:hidden px-6 pb-4`}>
        <a href="#about" className="block py-2 text-slate-300 hover:text-sky-400">About</a>
        ...
        {isLoggedIn && (
          <a href="/dashboard" className="block py-2 text-slate-300 hover:text-sky-400">Dashboard</a>
        )}
        ...
        <div className="border-t border-slate-700 mt-4 pt-4 flex flex-col space-y-3">
          {!isLoggedIn ? (
            <>
              ...
              <a href="/signup" className="bg-slate-700 text-white text-center font-semibold px-4 py-2 rounded-lg hover:bg-slate-600">
                Sign Up
              </a>
            </>
          ) : (
            <button
              onClick={handleLogout}
              className="bg-slate-700 text-white text-center font-semibold px-4 py-2 rounded-lg hover:bg-slate-600"
            >
              Logout
            </button>
          )}
